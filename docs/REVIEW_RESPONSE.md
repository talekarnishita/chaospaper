...
| **H3** | NHL: chaos possible; framework generalizes to continuous sports | **Supported** | `chaos_classification_results.txt` (0-1 test = chaotic), `LLE_reviewer6_results.txt`, `LLE_test_cases_summary.txt` (Team 52 LLE +0.04, consistent with 0-1 chaotic; LLE is supporting evidence — see Limitations) |
| **H4** | LLE and 0-1 test agree where both computed | **Supported** | Team 52: LLE > 0 and 0-1 = chaotic → `LLE_validation_report.txt` §3. Note: LLE is corroborating evidence; Rosenstein LLE can be small positive on stochastic data |
...
- We **do not** claim the methodology works identically for all sports. We state clearly: **NBA** (30 teams) was classified as **stochastic** under our pipeline; **NHL** (e.g. Team 52, 6, 14, 19) shows **chaotic** classification for some team-metric pairs under the 0-1 test, with positive LLE as supporting (not standalone) evidence. So the framework is validated on **ice hockey** (and synthetic chaos); **basketball** under our data and tests did not exhibit low-dimensional chaos. We frame this as **league/sport-dependent outcome** and discuss possible reasons (e.g. game structure, sampling, metric choice) in Discussion. We have extended NHL evidence (LLE test cases, `prove_hypothesis.m` for five teams) and cite `LLE_test_cases_summary.txt`, `prove_hypothesis_summary.csv`, and REVIEWER6_SUMMARY.txt.

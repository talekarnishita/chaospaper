...
%     gaussian_transform,surrogate_algorithm,downsampling_method,sigma)
%
%   A modified version of the 0-1 test for chaos classification with optional:
%       - stationarity test
%       - denoising
%       - normality transform
%       - surrogate-based stochasticity test
%       - oversampling check and optional downsampling
%
% INPUTS (same as original):
%   y  -- time-series vector
%   cutoff  -- The K-statistic threshold for chaos vs. periodic
%   stationarity_test  -- 'adf','kpss','lmc','vrat','bvr' or empty for none
%   denoising_algorithm -- 'schreiber','ma','wavelet' (default 'schreiber')
%   gaussian_transform  -- 0 (none), 1 (Box-Cox), 2 (rank-based inverse normal)
%   surrogate_algorithm -- e.g. 'IAAFT2','AAFT','FT','CPP','CSS','aaft_cpp','AAFT'
%   downsampling_method -- 'downsample' or 'fouda'
%   sigma  -- noise level in modified 0-1 test (default 0.5)
%
% OUTPUTS:
%   output.result = 'nonstationary','stochastic','chaotic','periodic'
...
h = 0;  % stationarity test flag (1 => nonstationary, 0 => stationary)

if nargin < 4 || isempty(denoising_algorithm)
    denoising_algorithm = 'schreiber';
end

if nargin < 5 || isempty(gaussian_transform)
    gaussian_transform = 0;
end

if nargin < 6 || isempty(surrogate_algorithm)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If data are found nonstationary (h=1), return immediately.
if h == 1
    output.result = 'nonstationary';
    % Provide empty placeholders so that fields exist
    output.K = [];
...
% Otherwise proceed:
    ...
    % Prepare output.denoised_data (initialize as empty).
    % We'll populate it if/when we actually do wavelet or other denoising.
    output.denoised_data = [];
    ...
    if strcmp(denoising_algorithm, 'schreiber')
        y = noiserSchreiber(y);
        if length(y) <= 20
            error('Not enough data for Schreiber + surrogate. Length = %d', length(y));
        end
        y = y(10:end-10); % cut off ends
        % If you want to store "schreiber"-denoised data, uncomment below:
        output.denoised_data = y;

    elseif strcmp(denoising_algorithm, 'ma') % moving average
        y = smooth(y);
        % If you want to store "ma"-denoised data, uncomment below:
        % output.denoised_data = y;

    elseif strcmp(denoising_algorithm, 'wavelet')
        disp('Using Wavelet denoising...');
        % Check if wdennoise exists in your MATLAB version
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Step 3: Test if data are stochastic using surrogate methods
    %         (and check linear vs. nonlinear via permutation entropy)
    
    % Optionally transform data to (approx) Gaussian for surrogate creation
    if gaussian_transform == 1
        surr_y = boxcox(y + abs(min(y)) + 0.0001); 
        % add offset so all values are positive
    ...
    if strcmp(surrogate_algorithm, 'aaft_cpp')
        ...
        sig = params.cutsig;
        surr_h2 = zeros(1,num_surr);
        for i = 1:num_surr
            surr_h2(i) = petropy(surr(i,:), 8, 1);
        end
        ...
        try
            [surr, params] = surrogate(surr_y, num_surr, surrogate_algorithm, 1, 1);
        catch
            [surr, params] = surrogate(zscore(surr_y), num_surr, surrogate_algorithm, 1, 1);
        end
        sig = params.cutsig;
        surr_h = zeros(1,num_surr);
        for i = 1:num_surr
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Step 4: If not stochastic, proceed to test chaos vs. periodic
    else
        ...
        if (max(y) - min(y)) / mean(abs(diff(y))) > 10
            ...
            elseif strcmp(downsampling_method, 'downsample')
                ...
                while oversample_flag == 1
                    y = downsample(y,2);
                    if (((max(y) - min(y)) / mean(abs(diff(y))) < 10) || (length(y) < 100))
                        oversample_flag = 0;
                    end
                end
            end
